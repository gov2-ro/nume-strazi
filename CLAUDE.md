...
11. **`streets_all_sources` is additive but not cross-source-deduplicated** — a street missing from the registry but corroborated by 2-3 external sources gets one row *per source* there. Use `all_street_names` (CODE_SPEC §13.8) for a genuinely deduplicated "every street name in Romania" list, with a `variants` JSON column preserving every source's exact spelling and a `corroboration_count`. Its external layer groups by `(siruta, street_type, core_name_norm)` — not `core_name_norm` alone — for the same type-matters reason as rule #1. No source (registry/OSM/postal/RENNS) is ranked above another; when sources disagree on spelling within a group, the representative name is just the alphabetically-first one.
