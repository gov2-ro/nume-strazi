...
--db override across all eleven steps without silently splitting writes across
...
Steps (in order, stop on first failure):
    ...
    8. tools/normalize_taxonomy.py (subcategory vocabulary safety net)
    9. tools/wikidata_persons.py --replay-csv --force
    10. tools/wiki_birthplace.py --replay-csv --force
    11. tools/wiki_biostats.py --replay-csv --force
...
NOTE on step 9: the replay CSV is the durable record of curated QIDs — whatever
...
def main():
    ...
    steps = [
        ...
        # Safety net. import_csv.py canonicalises subcategory on the way in, so
        # everything above should already be clean — but seed_lookups.py writes
        # its INSERTs directly, and a hand-edited DB can drift. Idempotent, so
        # running it when there is nothing to fix costs a single scan.
        ("tools/normalize_taxonomy.py", [py, "tools/normalize_taxonomy.py", "--db", DB]),
