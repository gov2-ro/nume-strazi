...
- [ ] **Fix relative links that break on detail pages** — Several paths are written as relative URLs that resolve correctly from the site root but break when browsing from subdirectory pages (`/strada/slug/`, `/persoana/slug/`, etc.). Known cases: `href="metodologie.html"` in the landing-page footer, `src="portraits/Q*.jpg"` in the foreigners panel JS renderer and the `renderFlat()` function, and potentially other asset references. Fix: audit all `href` and `src` values generated by both Jinja2 templates and client-side JS; convert relative paths to root-relative paths (starting with `/`). The site will be published at the domain root so `/`-prefixed paths will work everywhere. Grep targets: `href="metodologie`, `src="portraits/`, `href="${personHref}` in the JS block.
