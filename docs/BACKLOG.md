...
- [ ] **Trim DB further by column / row pruning** — `tools/build_dist_db.py` currently only drops empty/unused tables. After measuring real client query patterns, audit which `streets` / `streets_dedup` / `persons` columns are unused on the client side (e.g. `id`, `artera_raw`) and drop them before VACUUM. Also consider trimming `streets` to one row per (uat, name) instead of per polling-section (the view already does this; the underlying table doesn't need its 127k rows for the client). Target: <20 MB shipped DB.
