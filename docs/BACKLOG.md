...
- [ ] **Search index includes UATs without detail pages** — Follow-up from the UAT scoping above. `dist/cauta/uats.json` still indexes all 1207 UATs, so clicking a comună in search results 404s. Fix: either (a) filter `site_queries.explorer_indexes` to the same set as `enumerate_uats`, or (b) keep the index complete and mark indexed-but-unrendered UATs so the search-results UI hides or styles them differently. (a) is simpler; (b) preserves discoverability for the long tail.
