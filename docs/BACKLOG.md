...
- [ ] **Curation tooling (from CODE_SPEC P1)**
  ...
  - [ ] **QID rebuild persistence** — Manual QID assignments in DB don't survive `build_db.py` + `seed_lookups.py` rebuild. The CSV (`data/curation/wikidata_qids.csv`) is now the authoritative record. Add `--replay-csv` flag to `wikidata_persons.py` that reads the CSV and re-applies all confirmed rows (`auto_match=True`) to the DB, so a rebuild sequence is: `build_db.py → seed_lookups.py → seed_top500.py → seed_batch2.py → wikidata_persons.py --replay-csv`.
