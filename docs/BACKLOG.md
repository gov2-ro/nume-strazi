...
- [ ] **Make `all_street_names` (cross-source union) the dashboard's main corpus, replacing `streets_dedup` (registry-only)** — Decided 2026-07-14: the registry is no longer to be treated as the primary/authoritative source; all 4 sources (registry, OSM, postal, RENNS) are equal, and "how many streets does Romania have" should mean the deduplicated union (224,208), not the registry alone (107,957). This reverses the 2026-07-08 decision documented in activity-history ("the dashboard has always been, and remains, scoped to the AEP registry only, by design"). A code survey found this is a real multi-phase project, not a quick swap:
  - **Phase 0 (done 2026-07-14)** — `tools/materialize_all_street_names.py`: materializes the `all_street_names` view into an indexed `all_street_names_cache` table (siruta/core_name_norm/name_normalized indexes) in `data/streets.db`, since the raw view is ~5-8x slower than `streets_dedup` per query and lacks a diacritic-folded `name_normalized` column (the routing/slug key used 96× in `site_queries.py`). Purely foundational — invisible to the live dashboard, nothing rewired yet.
  - **Phase 1 (cleanup pass done 2026-07-14; classification run itself not started)** — Repoint `section1`'s headline stat and the ~20 column-compatible `site_queries.py` functions (`section2`, `section5`, `section6`/`section6_uat`, `section_lexical`, `enumerate_*`, `*_detail` functions, `browser_export`, `explorer_indexes`) at `all_street_names_cache`. **Must not start before running a classification pass** against the genuinely-new keys, so switching visible classification percentages doesn't crater them. Before spending LLM budget, ran a cheap cleanup pass first (see the two items above/below: postal comma-suffix fix + `resolve_reversed_person_duplicates.py`) — cut the "new" key count from 26,013 → 25,269 (744 resolved for free, plus a real data-quality fix to postal's title/rank extraction that improves everything downstream) and repointed `tools/export_unclassified.py`/`tools/llm_classify.py` at `all_street_names_cache` so they actually reach the new keys (previously scoped to `streets_dedup` only — would have silently missed all of them). Next step: run `tools/llm_classify.py --limit 0` (or in batches) against the remaining 25,269, then this phase's dashboard rewiring can start.
  - **Phase 2 (scoping decision, not a blocker)** — The OSM-km-linked panels (`total_km_per_honoree`, `gender_km_gap`, `female_km_leaderboard`, prestige-hierarchy, self-honor-index) rely on `streets_dedup.id` joining to `street_osm_matches`/etc. — a registry-street-to-OSM-segment linkage that has no equivalent against a merged union row (OSM length data is inherently OSM-specific, not a "which corpus" question). These stay explicitly scoped to "registry streets with a known OSM match," clearly captioned as such, regardless of what the headline corpus is.
  - **Phase 3 (deferred, low priority)** — 46 of 50 named queries in `docs/queries.sql` reference `streets_dedup`; each would need a hand-written `all_street_names_cache` variant (not mechanical find/replace, several depend on `name_normalized` or `id`-based match joins). Fine to do incrementally — this is the offline query catalog, not the live dashboard.
...
- [ ] research/ingest other sources?
  - [ ] create ingestion pipeline guide
  - [ ] Look into [osmnames](https://osmnames.org/download/), [openstreetdata](https://openstreetdata.org/#country-dumps), [overturemaps](http://overturemaps.org/) 
  - [ ] other similar alternative sources? 
