...
- [ ] **Fix `build_site.py --detail-only` hanging on re-runs** — Observed 2026-05-17: after a successful first full build, subsequent `build_site.py --detail-only` runs hang with no stdout output for many minutes (process burns CPU but never writes the first "Rendering N street detail pages…" line). Suspect SQLite locking, stdout buffering, or initialization-phase issue. Workaround for now: full rebuild via `build_site.py --detail`. Investigate: try `python3 -u` (unbuffered), check for lingering `sqlite3.Connection` handles, time the enumerator queries in isolation.
