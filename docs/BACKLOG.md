...
- [ ] **Make `all_street_names` (cross-source union) the dashboard's main corpus, replacing `streets_dedup` (registry-only)** — Decided 2026-07-14: the registry is no longer to be treated as the primary/authoritative source; all 4 sources (registry, OSM, postal, RENNS) are equal, and "how many streets does Romania have" should mean the deduplicated union (224,208), not the registry alone (107,957). This reverses the 2026-07-08 decision documented in activity-history ("the dashboard has always been, and remains, scoped to the AEP registry only, by design"). A code survey found this is a real multi-phase project, not a quick swap:
  ...
  - **Phase 1 (blocked on classification pass, not started)** — Repoint `section1`'s headline stat and the ~20 column-compatible `site_queries.py` functions (`section2`, `section5`, `section6`/`section6_uat`, `section_lexical`, `enumerate_*`, `*_detail` functions, `browser_export`, `explorer_indexes`) at `all_street_names_cache`. **Must not start before running a classification pass** (`tools/llm_classify.py` or equivalent) against the 26,013 `core_name_norm` keys that exist only in OSM/postal/RENNS and were never in the registry (29,302 → 55,315 distinct keys; a sample found 0% of the new keys currently classified) — switching visible classification percentages (55% natură, gender split, theme distribution, etc.) before that would roughly halve every coverage number shown on the site. This is the user's explicit decision, not a technical default.
...
- [ ] Should we also look into [osmnames.org](https://osmnames.org/download/) and/or other similar alternative sources?
