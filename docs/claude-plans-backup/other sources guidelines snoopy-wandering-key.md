# Write docs/INGESTION_PIPELINE.md — reference doc for adding new street-name sources

## Context

The project currently combines 4 independent sources of Romanian street
names (AEP polling-section registry, OpenStreetMap, Poșta Română postal
codes, ANCPI's RENNS cadastral registry) into one deduplicated master list,
`all_street_names`. The user wants to open the door to adding more sources
and asked, as a first step, for a written doc capturing the ingestion
pipeline pattern as it exists today — so a future session (or a future
prompt in this one) can use it as a checklist/reference instead of
re-deriving the pattern by reading 6 files from scratch.

This is a pure documentation task — no code changes. The plan below **is**
the doc content, to be written to `docs/INGESTION_PIPELINE.md` once
approved.

## Placement decision

New standalone file, `docs/INGESTION_PIPELINE.md`, not a `CODE_SPEC.md`
section. Reasoning: `CODE_SPEC.md` is described in `CLAUDE.md` as "full PRD
for the data layer" (schema rationale, curation strategy, prioritized
tasks) — a project-history document. This doc is different in kind: a
reusable operational pattern/checklist, read only when someone is actively
adding a new source, mirroring how `DESIGN_BRIEF.md` is already scoped
("only when work touches editorial decisions"). I'll add one line to
`CLAUDE.md`'s "Read first" list: *"`docs/INGESTION_PIPELINE.md` — only when
adding a new street-name source. Otherwise, skip."*

## Doc content (final — this is what gets written)
...
---
    ...
    Every external source goes through the same 4 stages, each its own
    script in `tools/`:
    ...
    **Required columns** (every `<source>_streets` table has all of these —
    copy `postal_streets`' or `renns_streets`' `CREATE TABLE` in
    ...
    **Person names may be in "Surname Firstname" order.** Postal's `Denumire
    ...
    2. Per-UAT registry coverage (% of `streets_dedup` rows in that UAT with
       ...
       before assuming it's a source gap (see Gorj/Tulcea note below: a low
       number is sometimes real sparse source coverage, not a bug).
    ...
    **Never blindly reorder two-token names to "fix" apparent duplicates.**
    A postal comma-suffix cleanup pass in this same session first tried
    reordering every 2-token "Surname Firstname" postal name to
    "Firstname Surname" — and corrupted real data doing it: "Gala Galaction"
    is a pen name, not a reversed surname/firstname pair, and "Petöfi
    Șándor" is Hungarian family-name-first order, *already correct*. Both
    got silently flipped to garbage. The fix: never reorder on a heuristic.
    Only reorder when the reversed form is a *confirmed, already-curated*
    identity (see `tools/resolve_reversed_person_duplicates.py`) — evidence,
    not a guess, and scoped to `persons` only (reordering has no meaning for
    a nature term or place name).
    ...
    3. `CREATE TABLE <source>_streets` — copy `postal_streets`'/
       `renns_streets`' shape, grain `(uat_siruta, name_normalized)`.
    ...
    12. Update `CLAUDE.md`'s critical rules if the new source breaks a new
        kind of assumption (as OSM/postal/RENNS each did) and the repo
...
Documentation-only — no code to test. "Verification" here means the doc
itself is accurate: every claim above is drawn from this session's direct
reading of `build_db.py`, `streets_lib.py`, `tools/osm_ingest.py`,
`tools/osm_match.py`, `tools/osm_sanity.py`, `tools/postal_ingest.py`,
`tools/postal_match.py`, `tools/renns_ingest.py`, `tools/renns_match.py`,
`tools/renns_sanity.py`, `tools/materialize_all_street_names.py`,
`tools/build_dist_db.py`, `tools/resolve_reversed_person_duplicates.py`,
and `CLAUDE.md`'s critical rules — not paraphrased from memory. After
writing the file, spot-check by grepping the doc's concrete claims (schema
column lists, `REFERENCE_UATS` values, match-tier names) against the
actual source files to confirm nothing drifted while drafting.
