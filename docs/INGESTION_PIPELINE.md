...
tools/<source>_match.py    <source>_streets ↔ electoral_dedup
tools/<source>_sanity.py   eyeball coverage on 5 reference UATs
(all_street_names view picks it up automatically — see below)
```

The electoral source itself (`build_db.py`) is the one source that
doesn't follow this pattern — it's ingested once, not a "new source" in
this sense, purely because it predates the other 3 (procedural, not a
data-authority claim — see CLAUDE.md's critical rules: no source
outranks another).

## Stage 1: `<source>_ingest.py`

**Grain**: one row per `(uat_siruta, name_normalized)` — same as every
other source table and `electoral_dedup`. If the raw source has
...
street_type        TEXT                 -- electoral-source-convention type word, or NULL
...
the electoral source's own ETL uses. `core_name_norm = normalize_match(core_name)`.
...
- RENNS: `uat.id` *is* the electoral source's SIRUTA directly (verified 3180/3181
...
Links source rows to electoral-source rows (`electoral_dedup`), *separate from*
...
`electoral_<source>_gap`, `<source>_only_streets` queries) and,
...
`name_normalized` includes the street-type prefix; the electoral source's,
...
artera` frequently is (`"Alecsandri Vasile"` vs. the electoral
source's `"Vasile Alecsandri"`) — hence Pass 3. RENNS and OSM haven't
needed this (their conventions already follow the electoral source's
Firstname-Surname order). **Don't assume a new source needs this pass — check a sample
...
1. Top-N by whatever the source's own quality signal is (OSM:
   ...
   the top.
2. Per-UAT electoral coverage (% of `electoral_dedup` rows in that UAT with
...
all_street_names`) is a `UNION ALL` across `electoral_dedup` +
...
`in_electoral` flag, and `judet`/`uat` labeling (via `uat_reference`) all
apply automatically.
...
one real bug**: a first design gave the electoral arm a type-blind
...
- `python3 tools/materialize_all_street_names.py` — the live
  ...
  (~5-8x slower per query than `electoral_dedup`), and has no
...
- A new source introduces brand-new `core_name_norm` keys the
  ...
  query `all_street_names_cache`, not `electoral_dedup` — a new source's
...
`electoral_dedup`) into real tables in the *copy* at `dist/streets.db`,
...
11. New `docs/queries.sql` block: `electoral_<source>_gap`,
