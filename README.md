...
- Provides a named SQL query catalog covering overview, people, themes,
  regional maps, renaming history, and curiosities
...
```bash
pip install openpyxl anthropic   # openpyxl: ETL only; anthropic: LLM classifier only
```
...
└── data/
    ├── reference/        # Source xlsx files (not committed — download separately)
